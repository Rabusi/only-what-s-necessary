...
class ConvLSTMAE(nn.Module):
    """
    PyTorch equivalent of your Keras model:

    TimeDistributed(Conv2D(128, 11, stride=4, same)) + LN
    TimeDistributed(Conv2D(64,  5, stride=2, same)) + LN

    ConvLSTM2D(64, 3, same, return_sequences=True) + LN
    ConvLSTM2D(32, 3, same, return_sequences=True) + LN
    ConvLSTM2D(64, 3, same, return_sequences=True) + LN

    TimeDistributed(Conv2DTranspose(64,  5, stride=2, same)) + LN
    TimeDistributed(Conv2DTranspose(128, 11, stride=4, same)) + LN
    TimeDistributed(Conv2D(1, 11, sigmoid, same))
    """
    def __init__(self, in_channels: int = 1):
        super().__init__()

        # TimeDistributed Conv2D == apply conv to each frame: just reshape (B*T, C, H, W)
        ...
        # Decoder: ConvTranspose2d matches strides; "same" can be slightly off by 1 pixel
        ...
        self.out_conv = nn.Conv2d(128, 1, kernel_size=11, padding=5)
        self.out_act = nn.Sigmoid()

    def _time_distributed_conv(self, x, conv2d):
        # x: (B, T, C, H, W) -> (B*T, C, H, W) -> conv -> (B, T, C2, H2, W2)
        B, T, C, H, W = x.shape
        y = conv2d(x.reshape(B * T, C, H, W))
        C2, H2, W2 = y.shape[1], y.shape[2], y.shape[3]
        return y.reshape(B, T, C2, H2, W2)

    def _time_distributed_norm(self, x, norm2d):
        # apply norm frame-wise
        B, T, C, H, W = x.shape
        y = norm2d(x.reshape(B * T, C, H, W))
        return y.reshape(B, T, C, H, W)

    def _time_distributed_deconv(self, x, deconv2d):
        B, T, C, H, W = x.shape
        y = deconv2d(x.reshape(B * T, C, H, W))
        C2, H2, W2 = y.shape[1], y.shape[2], y.shape[3]
        return y.reshape(B, T, C2, H2, W2)
    ...
    def forward(self, x):
        ...
        # output conv + sigmoid, frame-wise
        B, T, C, H, W = x.shape
        y = self.out_conv(x.reshape(B * T, C, H, W))
        y = self.out_act(y)
        return y.reshape(B, T, 1, y.shape[2], y.shape[3])
